...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        hout, hcache = affine_forward(X, W1, b1)
        haout, hacache = relu_forward(hout)
        scores, scores_cache = affine_forward(haout, W2, b2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dha, dW2, db2 = affine_backward(dlogits, scores_cache)
        dh = relu_backward(dha, hacache)
        dX, dW1, db1 = affine_backward(dh, hcache)
        grads["W1"] = dW1 + reg * W1
        grads["b1"] = db1
        grads["W2"] = dW2 + reg * W2
        grads["b2"] = db2
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            # dropout
            if self.use_dropout:
                out, cache = dout_forward(out, self.dropout_param)
                layers_cache.append(cache)
        ...
        for i in reversed(range(self.num_layers - 1)):
            # dropout
            if self.use_dropout:
                dX = dout_backward(dX, layers_cache.pop())
